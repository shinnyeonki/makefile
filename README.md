...
DESCRIPTION
    ...
    헤더 파일 의존성을 자동으로 분석하여 관련 소스 파일을 함께 컴파일합니다.
...
ADVANCED VARIABLES : 일반적으로 인수로 지정하지 않고 Makefile 내에서 설정합니다. 하지만 필요에 따라 인수로 지정할 수 있습니다.
    ...
    INCLUDE_DIRS+=dir
        추가 헤더 파일 디렉토리를 지정합니다. 여러 디렉토리를 추가적으로 지정할 수 있습니다.
        예: make build INCLUDE_DIRS+=include/ INCLUDE_DIRS+=third_party/
